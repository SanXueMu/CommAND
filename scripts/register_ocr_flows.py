...
INPUT_SCHEMAS: dict[str, dict] = {
    "flow.ocr.recognize": {
        ...
        "properties": {
            ...
            "output_db": {"type": ["string", "null"], "title": "结果库名", "description": "留空自动按文件名建库（<文件名>.ocr_results.db）"},
            "key_name": {"type": "string", "title": "密钥名称", "format": "keys",
                         "description": "设置页已录入的 DashScope 密钥名"},
            "model": {"type": ["string", "null"], "title": "多模态模型", "description": "留空用默认模型"},
            ...
            "force": {"type": ["boolean", "null"], "title": "重新识别（忽略已有结果）",
                      "description": "清掉该文件在本库的旧结果再全量识别（改过模版/规则后用）"},
